...
- Archive 與 close state 在同一 transaction 建立／提交，保存由 `startedAt` 產生的 immutable locale-independent `sessionLabel`，且 `purgeAt = closedAt + 90 days`；transaction 會把 Participant 的 account、displayName 與 token lookup fields 去識別。retention 在 `purgeAt` 當下可執行，若先於 admin confirmation 完成，會用 canonical retention event resolve pending request 並自 queue 移除。Checkpoint 1 只凍結 service/HTTP contract；runner、scheduler、dry-run 與 operational purge 屬 Checkpoint 2。
- payload 是匿名 aggregate：active detail 會把 persisted/legacy JSON 以 nested strict allowlist 重新建構，只輸出 schemaVersion、ordered questions 與 poll/quiz/open-text aggregate 欄位；未知欄位不會穿透 HTTP。open-text response 僅 `{ text }`。這是 projection-level de-identification；Submission 在 governed purge 前可仍以 FK 指向已匿名化 Participant，但 API 不得輸出 participant/account/displayName、token/sessionCode、idempotency key、submission timestamp 或 answer-to-person linkage。
...
- BE-5 CP1 remediation（2026-09-10）：strict persisted-payload allowlist、Teacher/Admin reason binding、conflicting replay 409、expanded OpenAPI/authorization/security/race contract 已完成 targeted verification。Machine-readable evidence：governance units **2 suites / 15 tests**、OpenAPI **1 suite / 5 tests**、guarded `smartlearning_test` archive governance E2E **1 suite / 11 tests**，全部 `0 failed / 0 skipped`；test DB migration status up to date；typecheck/lint/format/build/diff check PASS。仍待 CP1 人工 final sign-off；CP2 operational readiness 與 FE-6 不因本 evidence 自動完成。
