...
- `GET /api/v1/results?page=1&pageSize=20&courseId=<uuid>&liveSessionId=<uuid>&status=active|deleted&closedFrom=<ISO-date-time>&closedTo=<ISO-date-time>`：所有 query 均 optional。`closedFrom`／`closedTo` 必須是帶 `Z` 或明確 numeric offset 的 ISO date-time，分別以 `closedAt >= closedFrom` 與 `closedAt <= closedTo` 套用 inclusive instant range；`closedFrom > closedTo` 回標準 `400 VALIDATION_FAILED`。teacher 僅可讀自己課程，admin 可讀全部，student 回 403；foreign-owner filter 回 empty page，foreign detail 以 404 隱藏存在性。ownership 與 Course/LiveSession/status/closedAt filters 在 count 及 pagination 前共用，依 `closedAt DESC, id DESC` 穩定排序。row 為 `{ id, liveSessionId, course:{id,name}, sessionLabel, startedAt, closedAt, status, purgeAt, deletionRequest, deletion }`，不含 archive payload 或帳號/participant/token/submission linkage。
...
- `POST /api/v1/results/:liveSessionId/deletion-requests`：僅 teacher/course-owner；需 CSRF + exact Origin，body `{ reason:"privacy|support" }`。分層授權契約：route guard 為 TeacherOrAdmin（共用 guard），service 層才是 teacher-only 權威檢查（admin 經 admin confirmation path 消費 request，不得新增 admin 直接建立 request 的依賴）。同場 outstanding request 的 sequential/concurrent retry 回原 receipt `{ id, liveSessionId, reason, status:"requested", requestedAt }`，不改寫原 reason/time。
...
- `POST /api/v1/admin/results/:liveSessionId/deletion`：admin 需 CSRF、`StepUpGuard` 與 `{ deletionRequestId:<uuid>, confirmed:true, reason:"privacy|support" }`。request/session 綁定後，Admin reason 必須與原 Teacher request reason 完全一致；不一致（含 resolved request 的 conflicting replay）在任何 destructive write 前回 HTTP 409 `CONFLICT`、`field:"reason"`。matching replay 回同一 `DeletionResultDto`，不產生第二次刪除事件；request ID 不存在或屬於其他 session 仍以 404 existence-hiding。
- Archive 是 manual/automatic LiveSession close command 的同 transaction side effect；沒有獨立 HTTP archive create/finalize route。Archive 與 close state 在同一 transaction 建立／提交，保存由 `startedAt` 產生的 immutable locale-independent `sessionLabel`，且 `purgeAt = closedAt + 90 days`；transaction 會把 Participant 的 account、displayName 與 token lookup fields 去識別。replay 回既有 archive，不重建或替換 payload，並再次套用 Participant 去識別。retention 在 `purgeAt` 當下可執行，若先於 admin confirmation 完成，會用 canonical retention event resolve pending request 並自 queue 移除。Checkpoint 1 只凍結 service/HTTP contract；runner、scheduler、dry-run 與 operational purge 屬 Checkpoint 2。
- Archive payload 對 poll/quiz 僅保存匿名 aggregate，不保存 per-submission raw answer rows；open-text archive responses 僅保存匿名 raw `{ text }`。Active detail 會把 persisted/legacy JSON 以 nested strict allowlist 重新建構，只輸出 schemaVersion、ordered questions 與各題型允許欄位；未知欄位不會穿透 HTTP。這是 projection-level de-identification；Submission 在 governed purge 前可仍以 FK 指向已匿名化 Participant，但 API 不得輸出 participant/account/displayName、token/sessionCode、idempotency key、submission timestamp 或 answer-to-person linkage。
- 所有回應仍使用通用 envelope；沒有 student history/archive route。Retention scheduler/worker 的 operational readiness、dry-run 與 production enablement屬 Checkpoint 2，不是 FE-6 或本 CP1 contract closeout。
- Early-deletion tombstone 的 backup-restore 防護（BE-5.3.5/5.3.6）是 **operator 端機制**，不是前端 API：已刪資料經 manifest reconciliation（`retention reconcile-apply` CLI）再次移除，restore 完成 = reconciliation apply 結束且驗證查詢通過。前端唯一可見的契約即上方的 deleted tombstone form（`deletion` 欄位、無 `payload` key）。
...
- BE-5 CP1 remediation（2026-09-10）：strict persisted-payload allowlist、Teacher/Admin reason binding、conflicting replay 409、expanded OpenAPI/authorization/security/race contract 已完成 targeted verification。Machine-readable evidence：governance units **2 suites / 15 tests**、OpenAPI **1 suite / 5 tests**、guarded `smartlearning_test` archive governance E2E **1 suite / 11 tests**，全部 `0 failed / 0 skipped`；test DB migration status up to date；typecheck/lint/format/build/diff check PASS。CP2 operational readiness 與 FE-6 不因本 evidence 自動完成。
- BE-5.1 Checkpoint D（2026-09-11）：projection/privacy/replay units **2 suites / 16 tests** PASS；strict nested prohibited-field coverage、existing-archive no-create/no-update replay 與 participant anonymization replay regression 已加入。Checkpoint D 未新增 DB-backed replay run；close/archive/anonymization/outbox atomicity 仍以 shared transaction call-graph evidence 支持，沒有 failure-injection runtime proof。此紀錄基於 backend HEAD `1c841ca` 加 dirty working tree changes，尚未形成新的 immutable commit。
